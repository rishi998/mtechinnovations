      ...
      <div className="container-custom">
        <div className="grid grid-cols-1 gap-12 lg:grid-cols-2 lg:gap-16 lg:items-start">
          <div className="max-w-xl">
            ...
            <p className="mt-5 break-safe text-base leading-[1.75] text-ds-text-secondary">
            ...
            <Link
              href="/categories/"
              className="mt-8 inline-flex min-h-10 rounded-lg border border-ds-border bg-ds-surface px-6 py-3 text-sm font-semibold uppercase tracking-wide text-ds-text-primary transition duration-250 ease-out hover:border-ds-accent hover:brightness-110"
            >
              Browse the catalog
            </Link>
          </div>
          ...
          <div className="grid max-w-xl gap-4 lg:justify-self-end">
            {cases.map((item) => (
              <div key={item.title} className={cardClass}>
                <h3 className="break-safe text-base font-semibold text-ds-text-primary">{item.title}</h3>
                <p className="mt-2 break-safe text-sm leading-relaxed text-ds-text-secondary">{item.description}</p>
              </div>
            ))}
          </div>
        </div>
